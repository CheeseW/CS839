...
struct CGSystemWrapper
{
    ...
    {
        for (auto& v : y.m_data) v = AttributeType::Zero();
        m_finiteElements.addProductWithStiffnessMatrix( x.m_data, y.m_data, T(1.) );
    }
    ...
    void Project(VectorWrapper& y) const
    {
        m_finiteElements.clearConstrainedParticles(y.m_data);
    }
};
